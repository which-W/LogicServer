...
void CSession::Start() {
	...
	boost::asio::async_read(_socket, boost::asio::buffer(_recv_head_node->_data, HEAD_TOTAL_LEN), std::bind(&CSession::HandleReadHead, this,
		std::placeholders::_1, std::placeholders::_2, SharedSelf()));
}
//使用IOthreadPool时
//void CSession::Start() {
...
void CSession::HandleReadMsg(const boost::system::error_code& error, size_t  bytes_transferred,
	...
	if (!error && bytes_transferred == _recv_msg_node->_total_len) {
		_recv_msg_node->_data[_recv_msg_node->_total_len] = '\0';
		//std::cout << "recive data is" << _recv_msg_node->_data << endl;
		LogicSystem::GetInstance()->PostMsgToQueue(std::make_shared<LogicNode>(shared_from_this(), _recv_msg_node));
		//再次接收头部数据
		_recv_head_node->Clear();
